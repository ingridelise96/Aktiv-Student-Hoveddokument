\documentclass[11pt,a4paper,twoside]{report} % DO NOT CHANGE, UNLESS IF CHANGING STYLE

\usepackage{graphicx,subfigure,listings,url,paralist,tabularx,rotating,longtable,lscape,tabu}
\usepackage{layouts,hyperref,fancyhdr,listings,color,times,lipsum}
\usepackage[utf8]{inputenc} 
\usepackage[norsk]{babel} 
\usepackage[absolute]{textpos}
\usepackage{hiofreport}
\usepackage{makeidx}
\usepackage{pdfpages}
\usepackage{url}
\usepackage{float}
\usepackage{caption}
\usepackage{dirtytalk}
\usepackage{verbatim}
\usepackage{wrapfig}
\usepackage{soulutf8}
\usepackage[T1]{fontenc}

\input{meta}       % Use this to fill in author, title and other metadata used on front page
\input{defs}        % DO NOT CHANGE

%\makeindex

\newcommand{\skisser}{skisser/Illustrasjoner/Skisser}
\newcommand{\tittel}[1]{\paragraph{#1}\mbox{}\\}
\newcommand{\largetittel}[1]{\subsubsection{\large{#1}}}
\newcommand{\fet}[1]{{\bf #1} \newline}

\setlength{\parskip}{1em}

\begin{document}

% FRONT MATTER
    % DO NOT CHANGE
    %-------------------------------------------------    
    \includepdf[pages={1, {}}] {Illustrasjoner/aktivstudent-forside.pdf} % Make your own catchy PDF title page, named titlepage.pdf
    \maketitle
    \cleardoublepage % TO BE REMOVED IN FINAL REPORT
    \input{abstract}
    \input{acknowledgements}
    %-------------------------------------------------

	\cleardoublepage % DO NOT CHANGE    
	\setcounter{tocdepth}{1}
    \tableofcontents  % DO NOT CHANGE    
    \cleardoublepage % DO NOT CHANGE    
    
    % OPTIONAL
    %-------------------------------------------------
    \listoffigures
    \addcontentsline{toc}{chapter}{Figurliste}
    \cleardoublepage
    \listoftables
    \addcontentsline{toc}{chapter}{Tabelliste}
    \cleardoublepage
    %-------------------------------------------------
       
    \pagenumbering{arabic} % DO NOT CHANGE, OR REMOVE IF CHANGING STYLE
    \setcounter{page}{1}       % DO NOT CHANGE, OR REMOVE IF CHANGING STYLE 		
	
% BODY
    % YOU MAY CHANGE TITLES AND ORDER (WITHIN LIMITS)
    %-------------------------------------------------
    \input{Kapitler/introduction}
    \input{Kapitler/method}
    \input{Kapitler/results}
    \input{Kapitler/discussion}
    %-------------------------------------------------


% BACK MATTER
    % DO NOT CHANGE
    %-------------------------------------------------   
    \bibliographystyle{apalike}
    \bibliography{project_bibliography}
    \addcontentsline{toc}{chapter}{Bibliografi}
    %-------------------------------------------------
        
    % OPTIONAL
    %-------------------------------------------------
    \cleardoublepage    
    %To make a good index is quite laborsome, and perhaps a bit outside the scope in this context    
    %\printindex
    \appendix
    \input{skisser1}
    \input{skisser2}
    \input{skisser3}
    \input{prosjektbeskrivelse}
    %-------------------------------------------------
    
    \cleardoublepage %DO NOT CHANGE
    \blankpage          %DO NOT CHANGE
    \blankpage          %DO NOT CHANGE
\end{document}
